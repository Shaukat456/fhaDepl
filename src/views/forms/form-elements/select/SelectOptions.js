...
import { GET_ALL_CONSUMER, PRODUCER } from '../../../../@core/auth/req.config'
...
export const  AsyncSelectProducer=( {api_url , title,demoData})=>{
...
const loadOptions=()=>{
   softRequest(PRODUCER,"GET",true,true).then((res)=>{
    console.log(res)
   }).catch(e=>{
    console.log(e)
   })
    ...
    return (
      ...
      <Col md={6} xs={12} className='mb-1 w-100'>
      <Label>  {title}  </Label>
      <AsyncSelect
        isClearable={false}
        className='react-select'
